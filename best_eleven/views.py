...
class BestElevenBuilderView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Ambil 'load' dari parameter URL dan kirim ke template
        ...
        for item in player_ids_data:
            ...
            if player_id_str and slot_id_str:
                try:
                    ...
                    print(f"Invalid UUID format: {player_id_str}")
            else:
                print(f"Incomplete slot data: {item}")
...
# --- Ambil atau Hapus Formasi ---
@login_required
@csrf_exempt
@require_http_methods(["GET", "DELETE"])
def get_formation_details_api(request, pk):
    """Ambil detail formasi (GET) atau Hapus formasi (DELETE)"""
    try:
    ...
    if request.method == 'DELETE':
        ...
        players_map = {
            str(p.id): p
            for p in Player.objects.filter(id__in=[i['playerId'] for i in valid_slot_items])
            .prefetch_related('current_club')
        }
